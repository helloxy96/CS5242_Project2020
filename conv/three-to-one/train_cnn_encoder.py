...
from Models.CNN_Encoder_old import encoder, single_logits_classifier, three_logits_classifier
...
# dataset
dataset = FileDataset(data_feat, data_labels, chop_num = 3)
...
valid_dataset = FileDataset(valid_feat, valid_labels, seed = 2, chop_num = 3)
...
for epoch in range(epochs):
    ...
    dataset = FileDataset(data_feat, data_labels, seed=seed, chop_num = 3)
    train_loader = tud.DataLoader(dataset, 
        batch_size=batch_size, 
        shuffle=True, 
        num_workers = 8, 
...
np.save(f'./results/cnn_encoder/training_losses_{timestamp}_orig.npy', A)
np.save(f'./results/cnn_encoder/training_scores_{timestamp}_orig.npy', B)

np.save(f'./results/cnn_encoder/test_losses_{timestamp}_orig.npy', C)
np.save(f'./results/cnn_encoder/test_scores_{timestamp}_orig.npy', D)

torch.save(encoder, f'./trained/cnn_encoder/encoder_{timestamp}_orig.pkl')
torch.save(single_logits_classifier, f'./trained/cnn_encoder/single_classifier_{timestamp}_orig.pkl')
torch.save(three_logits_classifier, f'./trained/cnn_encoder/three_classifier_{timestamp}_orig.pkl')
